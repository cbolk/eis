...
//t is the time interval value, could be either 15 min (0.25h) or 1h (1)
function prepare_powerdata(data, t){
...
$.each(names, function (i, name) {
  ...
  function powermeter_chart(powerdata){

    power = prepare_powerdata(powerdata,1);
    ...
    $('#powermeter_chart').highcharts({
      chart: {
        ...
        xAxis: {
              type: 'datetime',
                tickInterval: 2 * 3600 * 1000, // one week
                tickWidth: 0,
                gridLineWidth: 1,
                labels: {
                    align: 'left',
      ...
      if(jdata.length == 0){ //if no data display message
        $('#powermeter_chart').highcharts().showLoading("No data to display ):");
      }
